...
export function RecentRecords({ records, viewAllLink }: RecentRecordsProps) {
  return (
          ...
          <tbody>
            {records.map((record) => (
              <tr key={record.id}>
                ...
                <td className="text-muted-foreground text-sm">{new Date(record.createdAt).toLocaleDateString()}</td>
              </tr>
            ))}
            {records.length === 0 && (
              <tr>
